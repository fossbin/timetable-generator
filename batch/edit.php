<?php 
    ...
    $bid = $_GET['bid'];
    $sql1="select * from tbl_batch where bid=$bid";
	...
	{    
            ...
            $newbTo=$_POST['bTo'];
            $newbCurrentSem=$_POST['bCurrentSem'];
            $newbStatus=$_POST['bStatus'];
            
                $sql = "update tbl_batch set pid = '$newpid',bName = '$newbName',bFrom = '$newbFrom',bTo= '$newbTo',bCurrentSem = '$newbCurrentSem' where bid = '$bid'";
                                        ...
                                        <div class="form-group">
                                                ...
                                                <select name="pName" class="form-control" required>
													...
													$sql="select * from tbl_program";
                                                                    ...
                                                                    {
                                                        ?>
                                                        <option value="<?php echo $row["pid"]; ?>" <?php if($row['pid']==$pid){ ?>selected="selected"<?php } ?>> <?php echo $row["pName"] ;?> </option>
                                                        <?php
                                                        }
                                                        }
                                                    ?>  
											...
											<div class="form-group">
												<label for="exampleInputEmail1">Enter Batch</label>
                                               <!-- <input type="text" autofocus name="bName" class="form-control" placeholder="<?php echo $bName; ?>" required />-->
                                                <input type="text" autofocus name="bName" class="form-control"  required value="<?php echo $bName; ?>"/>
												<!--<p style = "color: red;"><?php echo $errMsg; ?></p>  --> 
											</div>
                                            <div class="form-group">
												<label for="exampleInputEmail1">Enter From Date</label>
                                                <input type="date" autofocus name="bFrom" class="form-control"  required value="<?php echo $bFrom; ?>" />
												<!--<p style = "color: red;"><?php echo $errMsg; ?></p>  --> 
											</div>
                                            <div class="form-group">
												<label for="exampleInputEmail1">Enter To Date</label>
                                                <input type="date" autofocus name="bTo" class="form-control"  required value="<?php echo $bTo; ?>" />
												<!--<p style = "color: red;"><?php echo $errMsg; ?></p>  --> 
											</div>
                            
                                            <div class="form-group">
												<label for="exampleInputEmail1">Select current semester</label>
                                               
                                                <select name="bCurrentSem" class="form-control" required>
                                                   <option value="<?php echo $bCurrentSem?>"><?php echo $bCurrentSem?></option>
                                                    <?php 
													
                                                    $sql="select semesters from tbl_program where pid=$pid";
                                                    $result=$db->query($sql);
                                                    if($result->num_rows >0)
                                                    { 
                                                        $row=$result->fetch_assoc();
                                                            for($i=1;$i<=$row['semesters'];$i++)
                                                            { 
                                                                if($i!=$bCurrentSem)
